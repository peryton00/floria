...
import { DomainMediaService } from "./media/domain-media.service.js";
...
async function runStage10Verification() {
  ...
  function createUniquePngBuffer(): Buffer {
    const basePng = Buffer.from(
      "89504e470d0a1a0a0000000d4948445200000001000000010802000000907753de0000000c4944415408d763f8cfc000000301010018dd8db0000000049454e44ae426082",
      "hex"
    );
  ...
  const prodSession = await MediaService.createUploadSession(sellerA, {
    profile: "PRODUCT",
  ...
  const { error: upErr1 } = await adminDb.storage.from("media-staging").upload(
    prodSession.stagingPath,
    prodImgBuf,
  ...
  const prodComp = await MediaService.completeUploadSession(sellerA, prodSession.sessionId);
  ...
  const logoSession = await MediaService.createUploadSession(sellerA, {
    profile: "SELLER_LOGO",
  ...
  await adminDb.storage.from("media-staging").upload(
    logoSession.stagingPath,
    logoBuf,
    { contentType: "image/png", upsert: true }
  );

  const logoComp = await MediaService.completeUploadSession(sellerA, logoSession.sessionId);
  await worker.processJob({
    data: {
  ...
  const logoRes = await DomainMediaService.updateSellerLogo(sellerA, logoComp.assetId);
  ...
  const avatarSession = await MediaService.createUploadSession(sellerA, {
    profile: "USER_AVATAR",
  ...
  await adminDb.storage.from("media-staging").upload(
    avatarSession.stagingPath,
    avatarBuf,
    { contentType: "image/png", upsert: true }
  );

  const avatarComp = await MediaService.completeUploadSession(sellerA, avatarSession.sessionId);
  await worker.processJob({
    data: {
  ...
  const avatarRes = await DomainMediaService.updateUserAvatar(sellerA, avatarComp.assetId);
  ...
  if (cat) {
    ...
    const bannerSession = await MediaService.createUploadSession(adminUser, {
      profile: "CATEGORY",
    ...
    await adminDb.storage.from("media-staging").upload(
      bannerSession.stagingPath,
      bannerBuf,
      { contentType: "image/png", upsert: true }
    );

    const bannerComp = await MediaService.completeUploadSession(adminUser, bannerSession.sessionId);
    await worker.processJob({
      data: {
    ...
    const categoryRes = await DomainMediaService.updateCategoryBanner(adminUser, cat.id, bannerComp.assetId);
  ...
  const docSession = await MediaService.createUploadSession(sellerA, {
    profile: "DOCUMENT",
  ...
  await adminDb.storage.from("media-staging").upload(
    docSession.stagingPath,
    pdfBuf,
    { contentType: "application/pdf", upsert: true }
  );

  const docComp = await MediaService.completeUploadSession(sellerA, docSession.sessionId);
  await worker.processJob({
    data: {
  ...
  const docRes = await DomainMediaService.attachSellerDocument(sellerA, "gstin", docComp.assetId);
  console.log("✅ Seller Document Registered in Private Bucket:", {
    documentId: docRes.id,
    file_asset_id: docRes.file_asset_id,
  });

  const signedUrlRes = await DomainMediaService.getSignedDocumentUrl(sellerA, docRes.id);
