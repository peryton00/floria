...
export const Errors = {
  ...
  validation: (msg: string) =>
    new ApiError("VALIDATION_ERROR", msg, 422),
  rateLimited: (msg = "Too many requests. Please slow down.") =>
    new ApiError("RATE_LIMITED", msg, 429),
  outOfStock: (name?: string) =>
    new ApiError("OUT_OF_STOCK", name ? `"${name}" is out of stock.` : "One or more items are out of stock.", 409),
  invalidTransition: (from: string, to: string) =>
    new ApiError("INVALID_STATUS_TRANSITION", `Cannot transition from "${from}" to "${to}".`, 409),
