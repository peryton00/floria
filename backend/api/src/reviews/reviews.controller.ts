...
export class ReviewsController {
  ...
  async getProductReviews(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const result = await reviewsService.getProductReviews(String(req.params.id), page, pageSize);
  ...
  async markHelpful(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const action = await reviewRepository.toggleHelpful(String(req.params.rid), req.user!.id);
  ...
  async getSellerReviews(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { sellerId } = req.user!;
      if (!sellerId) { res.status(403).json({ success: false, error: { code: "NOT_SELLER", message: "Seller profile not found." } }); return; }
      const page = Math.max(1, Number(req.query.page) || 1);
  ...
  async flagReview(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { sellerId } = req.user!;
      if (!sellerId) { res.status(403).json({ success: false, error: { code: "NOT_SELLER", message: "Seller profile not found." } }); return; }
      const ok = await reviewRepository.flagReview(String(req.params.id), sellerId);
      if (!ok) { res.status(404).json({ success: false, error: { code: "NOT_FOUND", message: "Review not found or not eligible to flag." } }); return; }
      res.json({ success: true, data: { flagged: true } });
    } catch (err) { next(err); }
  }

  // GET /api/v1/admin/reviews
  async adminListReviews(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
  ...
  async adminModerateReview(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const result = await reviewsService.moderateReview(String(req.params.id), action, note);
      if (!result) { res.status(404).json({ success: false, error: { code: "NOT_FOUND", message: "Review not found." } }); return; }
