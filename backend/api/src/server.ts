...
async function bootstrap() {
  try {
    ...
    try {
      ...
      logger.info("[MediaWorker] Background BullMQ queue worker started successfully.");
    } catch (err: any) {
      logger.warn(`[MediaWorker] Background worker initialization notice: ${err.message}`);
    }
