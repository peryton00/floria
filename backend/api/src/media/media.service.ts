...
export class MediaService {
   ...
   */
  public static async uploadDirectMedia(
    user: AuthenticatedUser,
    input: {
      filename?: string;
      mimeType?: string;
    ...
    const db = getAdminDb();
    if (!input.base64Data) {
      throw Errors.validation("Field 'base64Data' is required.");
    }
    ...
    if (buffer.length > MAX_FILE_SIZE_BYTES) {
      throw Errors.validation("Uploaded image size exceeds 10 MB ceiling.");
    }
