...
import { resolveSystemUploaderUserId } from "./migrate-system-assets.js";

export interface UnsplashAssetDefinition {
  id: string;
  url: string;
...
export interface MigratedUnsplashResult {
  originalUrl: string;
  assetId: string;
  sha256Hash: string;
  profile: string;
...
export const UNSPLASH_ASSETS_MANIFEST: UnsplashAssetDefinition[] = [
...
export async function migrateUnsplashAssets(
  ...
  const results: MigratedUnsplashResult[] = [];
