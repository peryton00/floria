...
class AdminFinancialService {
    async getProductFinancialCalculation(productId) {
    ...
    async getOrderFinancialBreakdown(orderId) {
        const db = (0, database_js_1.getAdminDb)();
        ...
        for (const it of items || []) {
            ...
            if (!sellerItemMap.has(sId))
                sellerItemMap.set(sId, []);
            sellerItemMap.get(sId).push(it);
        }
        const nurseryBreakdown = [];
