...
describe("Phase 3.23 Automated Protection — Hardcoded Pricing Value Guard", () => {
  ...
  it("ensures zero hardcoded commission or profit rate multipliers (* 0.12, * 0.02, * 12 / 100, * 2 / 100) in production source", () => {
    const forbiddenPatterns = [
      { pattern: /\*\s*0\.12\b/, desc: "Hardcoded 12% commission multiplier (* 0.12)" },
      { pattern: /\*\s*0\.02\b/, desc: "Hardcoded 2% profit multiplier (* 0.02)" },
      { pattern: /\*\s*12\s*\/\s*100/, desc: "Hardcoded 12% commission percentage calculation (* 12 / 100)" },
      { pattern: /\*\s*2\s*\/\s*100/, desc: "Hardcoded 2% profit percentage calculation (* 2 / 100)" },
    ];
  ...
  it("ensures zero hardcoded delivery threshold evaluations (>= 59900, >= 49900) in production source", () => {
    const forbiddenPatterns = [
      { pattern: />=\s*59900\b/, desc: "Hardcoded free delivery threshold check (>= 59900)" },
      { pattern: />=\s*49900\b/, desc: "Hardcoded legacy delivery threshold check (>= 49900)" },
      { pattern: />=\s*599\b/, desc: "Hardcoded rupee delivery threshold check (>= 599)" },
    ];
  ...
  it("ensures frontend never imports or executes client-side pricing formula calculations", () => {
    const forbiddenFrontendPatterns = [
      { pattern: /calculateCustomerProductPricePaise/, desc: "Client-side customer price calculation helper" },
      { pattern: /calculateSellerNetEarningsPaise/, desc: "Client-side seller net earnings calculation helper" },
    ];
  ...
  it("ensures backend services calculate pricing exclusively through PricingService", () => {
    ...
    for (const file of getFiles(apiSrcDir)) {
      ...
      if (/\bprice_paise\s*\*\s*\(/i.test(content) || /\bbase_price_paise\s*\*\s*\(/i.test(content)) {
        violations.push(`${relativePath}: duplicate pricing calculation logic found`);
      }
    }
