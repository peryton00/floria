...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  }); // keep in sync every render (no deps = always)
  ...
  const mapDbCartItems = useCallback((items: any[]): CartItem[] => {
    return items.map((ci: any) => {
      ...
      const customerPrice =
        ...
        inv.price_paise ??
        ci.unit_price_paise_snapshot ??
        0;
      const originalPricePaise =
        inv.original_price_paise && inv.original_price_paise > customerPrice
          ? inv.original_price_paise
          : null;
      const discountAmountPaise = originalPricePaise
        ? originalPricePaise - customerPrice
        : 0;
      const discountPercentage = originalPricePaise
        ? Math.round((discountAmountPaise / originalPricePaise) * 100)
        : 0;
          ...
          if (stored) {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
              const mergePayload = parsed
                .map((item: any) => ({
                  productId:
                    item?.listing?.product?.id || item?.productId || item?.id,
        ...
        if (res.success && res.data) {
          ...
          if (prevItems.length > 0) {
            ...
            for (const newItem of mapped) {
              ...
              const newPrice =
                newItem.listing.pricing?.sellingPricePaise ??
                newItem.listing.inventory?.price_paise ??
                0;
              if (
              ...
              .then((res) => {
                if (res.success && res.data && res.data.length > 0) {
                  ...
                  const updated = currentItems.map((item) => {
                    ...
                    const livePricing =
                      live.pricing ||
                      (Array.isArray(live.inventory)
                        ? live.inventory[0]?.pricing
                        : live.inventory?.pricing);
                    const newPrice =
                      livePricing?.sellingPricePaise ??
                      livePricing?.customerPricePaise ??
                      (Array.isArray(live.inventory)
                        ? live.inventory[0]?.price_paise
                        : live.inventory?.price_paise);
                    const oldPrice =
                      item.listing.pricing?.sellingPricePaise ??
                      item.listing.inventory?.price_paise ??
                      0;
  ...
  const addToCart = async (listing: ProductListing, qty = 1) => {
    toast.success(
      "Added to cart",
      `${listing.product.name} was added to your cart.`,
    );
    ...
    setCartItems((prev) => {
      const existing = prev.find(
        (item) => item.listing.product.id === listing.product.id,
      );
      if (existing) {
  ...
  const removeFromCart = async (productId: string) => {
    ...
    setCartItems((prev) =>
      prev.filter((item) => item.listing.product.id !== productId),
    );
  };
  ...
  const updateQuantity = async (productId: string, quantity: number) => {
    ...
    setCartItems((prev) =>
      prev.map((item) =>
        item.listing.product.id === productId ? { ...item, quantity } : item,
      ),
    );
  ...
  const subtotal = cartItems.reduce(
    (acc, item) =>
      acc + (item.listing?.inventory?.price_paise ?? 0) * item.quantity,
    0,
  );
