...
export default function OperationsOrdersPage() {
  ...
  const handleUpdateStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      toast.error(
        "Transition failed",
        e.message || "Error updating order status",
      );
    } finally {
      ...
      <div className="space-y-6">
        <div>
          <h1 className="font-serif text-2xl font-bold text-ink-900 leading-tight">
            Master Order Logistics
          </h1>
          <p className="text-xs text-ink-400 mt-0.5">
            Track multi-nursery consolidation and advance fulfillment state
            machine.
          </p>
        </div>
          ...
          <div className="bg-white rounded-xl border border-ink-100 shadow-sm overflow-hidden">
            {orders.length === 0 ? (
              <div className="p-12 text-center text-xs text-ink-400">
                No operational orders matching filter.
              </div>
            ) : (
            ...
            <div className="bg-white rounded-2xl border border-ink-100 p-6 max-w-md w-full shadow-2xl space-y-4">
              ...
              <div className="bg-cream-50 rounded-xl p-4 text-xs space-y-1">
                ...
                <p className="text-ink-700">
                  {selectedOrder.delivery_address_snapshot?.full_name}
                </p>
                <p className="text-ink-500">
                  {selectedOrder.delivery_address_snapshot?.line1},{" "}
                  {selectedOrder.delivery_address_snapshot?.city}
                </p>
              </div>
              ...
              <div className="space-y-2 pt-2">
                <p className="text-xs font-bold uppercase tracking-wider text-ink-700">
                  Allowed Operations State Machine Actions
                </p>
                ...
                {(selectedOrder.status === "picked up" ||
                  ...
                  <button
                    ...
                    onClick={() =>
                      handleUpdateStatus(selectedOrder.id, "packing")
                    }
                ...
                {selectedOrder.status === "packing" && (
                  <button
                    ...
                    disabled={actionLoading}
                    onClick={() =>
                      handleUpdateStatus(selectedOrder.id, "out for delivery")
                    }
                    className="w-full py-2.5 rounded-xl bg-forest-700 hover:bg-forest-800 text-white font-bold text-xs uppercase tracking-wider transition-colors disabled:opacity-50"
                  >
