...
export default function OperationsPackingPage() {
  ...
  const handleUpdatePacking = async (orderId: string, status: string) => {
    try {
      ...
      const res = await api.updatePackingTask(
        orderId,
        status,
        selectedTask?.items?.length || 1,
      );
      if (res.success) {
        toast.success(
          "Packing status updated",
          `Packing task updated to ${status}.`,
        );
        await fetchTasks();
        ...
        toast.error(
          "Update failed",
          res.error?.message || "Failed to update packing task",
        );
      }
    } catch (e: any) {
      ...
      <div className="space-y-6">
        <div>
          <h1 className="font-serif text-2xl font-bold text-ink-900 leading-tight">
            Central Packing Queue
          </h1>
          <p className="text-xs text-ink-400 mt-0.5">
            Consolidate multi-nursery plant items into secure customer packages.
          </p>
        </div>
          ...
          <div className="bg-white rounded-xl border border-ink-100 shadow-sm overflow-hidden">
            {tasks.length === 0 ? (
              <div className="p-12 text-center text-xs text-ink-400">
                No packing tasks currently in queue.
              </div>
            ) : (
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-serif text-lg font-bold text-ink-900">
                    Packing Order {selectedTask.orderId}
                  </h3>
                  <p className="text-xs text-ink-400 mt-0.5">
              ...
              <div className="space-y-2">
                ...
                <div className="bg-cream-50 rounded-xl p-3 space-y-2 text-xs">
                  {(selectedTask.items || []).map((item: any, i: number) => (
                    <div
                      key={i}
                      className="flex items-center justify-between border-b border-ink-100 pb-1"
                    >
                      <span className="font-semibold text-ink-900">
                        {item.product_name_snapshot || "Plant Item"}
                      </span>
