...
function LoginForm() {
  ...
  return (
    <div className="max-w-md mx-auto py-12 px-6 bg-white rounded-2xl border border-ink-100 shadow-sm animate-fade-in my-8">
      <h1 className="font-serif text-2xl font-bold text-ink-900 mb-1 text-center">Login to Floria</h1>
      <p className="text-xs text-ink-400 text-center mb-6">Welcome back! Please sign in to your account.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="login-email" className="block text-xs font-bold uppercase tracking-wider text-ink-500 mb-2">
            Email Address *
          </label>
          <input
            id="login-email"
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <label htmlFor="login-pass" className="block text-xs font-bold uppercase tracking-wider text-ink-500">
              Password *
            </label>
            <a href="#" className="text-xs text-forest-700 font-semibold hover:text-forest-900">Forgot password?</a>
          </div>
          <input
            id="login-pass"
            type="password"
      ...
      <p className="text-xs text-ink-400 text-center mt-6">
        ...
        <Link href={`/signup${nextParam ? `?next=${encodeURIComponent(nextParam)}` : ""}`} className="text-forest-700 font-semibold hover:text-forest-900">
          Sign up
        </Link>
      </p>
    </div>
...
export default function LoginPage() {
  return (
    <CustomerShell>
      <Suspense fallback={<div className="text-center py-20 text-xs text-ink-400">Loading...</div>}>
        <LoginForm />
      </Suspense>
    </CustomerShell>
  );
